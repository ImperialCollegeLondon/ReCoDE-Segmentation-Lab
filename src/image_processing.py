...
def otsu_threshold(array3d):
    ...
    This function implements Otsu's method, matching the behavior of
    scikit-image's `threshold_otsu`. It is suitable for integer-valued arrays such as
    uint8 or uint16, commonly used in image stacks.

    Mathematical background:
    Otsu's method selects the threshold that maximizes the between-class variance sigma_b^2(t),
    which quantifies how well the image is separated into foreground and background.

    This function uses the following vectorized formulation:
        sigma_b^2(t) = [mu_T · omega(t) - mu(t)]² / [omega(t) · (1 - ωomega(t))]
    where:
    ...
    This is mathematically equivalent to the more common form:
        sigma_b^2(t) = omega_b · omega_f · (mu_b - mu_f)²
    where:
    ...
    - Chityala, Ravishankar, and Sridevi Pudipeddi. "Image Processing and Acquisition Using Python. Second edition."
    Boca Raton: Chapman & Hall/CRC, 2020. Print.
    - Original paper: Otsu, N. (1979). "A threshold selection method from gray-level histograms."
      IEEE Transactions on Systems, Man, and Cybernetics, 9(1), 62-66.

    Parameters:
        array3d (np.ndarray): 3D array of shape (depth, height, width) containing integer voxel values.
    ...
    # Get the maximum possible value for the data type (e.g., 255 for uint8, 65535 for uint16 (typical for micro CT data))
...
def chamfer_distance_3d(img):
    ...
    which propagates integer-valued distances through local neighborhoods. It is suitable
    for binary 3D arrays where foreground voxels are non-zero and background voxels are zero.

    Mathematical background:
    The chamfer method uses a fixed set of weighted offsets to simulate Euclidean distances
    through iterative forward and backward passes. Each voxel updates its value based on
    neighboring voxels and predefined weights, approximating the shortest path to the nearest
    foreground voxel.
    ...
    The algorithm consists of two phases:
        - Forward pass: propagates distances from top-left-front to bottom-right-back
        - Backward pass: refines distances from bottom-right-back to top-left-front

    This method is significantly faster than exact Euclidean transforms and well-suited
    for large volumes where performance is critical.
    ...
    Parameters:
        img (np.ndarray): 3D binary array of shape (X, Y, Z) with 0 for background and non-zero for foreground.

    Returns:
        np.ndarray: 3D array of same shape with chamfer distances (integer approximation of Euclidean distance).
